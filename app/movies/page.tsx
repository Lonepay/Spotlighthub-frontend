...
function MoviesPageInner() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await movies.getPublicAll(params);
      setList(data.data || []);
  ...
  const selectCity = (c: string) => {
    setCity(c);
    setPage(1);
    router.replace(c ? `/movies?city=${encodeURIComponent(c)}` : '/movies');
  };
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="mb-10">
          <div className="text-xs uppercase tracking-widest text-primary-glow mb-2">Movies</div>
          <h1 className="text-4xl font-bold mb-3">What's showing</h1>
          <p className="text-lg text-muted-foreground">Pick a showtime, choose your seats</p>
        </div>
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {list.map((movie) => {
                ...
                return (
                  <Link key={movie.id} href={`/movies/${movie.id}`} className="group relative block">
          ...
          <div className="text-center py-20">
            <p className="text-muted-foreground text-lg">No movies with upcoming showtimes right now. Check back soon.</p>
          </div>
        )}
      </div>
